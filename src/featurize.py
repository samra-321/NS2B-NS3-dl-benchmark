...
def featurize_dataset(csv_path, smiles_col="Smiles", value_col="Standard Value",
                       ic50_cutoff_nm=1000):
    """Load raw ChEMBL CSV, derive binary activity labels from IC50 cutoff,
    and return fingerprint matrix X and labels y.

    Active (1): Standard Value <= ic50_cutoff_nm
    Inactive (0): Standard Value > ic50_cutoff_nm
    """
    df = pd.read_csv(csv_path)

    # Drop rows with missing SMILES or IC50 value
...
if __name__ == "__main__":
    X, y = featurize_dataset("data/NS2B_NS3_compounds.csv")
